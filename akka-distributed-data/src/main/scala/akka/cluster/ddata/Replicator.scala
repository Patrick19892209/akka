...
final class Replicator(settings: ReplicatorSettings) extends Actor with ActorLogging {
  ...
  def receiveWrite(key: String, envelope: DataEnvelope): Unit = {
    write(key, envelope) match {
      case Some(newEnvelope) ⇒
        if (isDurable(key))
          durableStore ! Store(key, new DurableDataEnvelope(newEnvelope), Some(StoreReply(WriteAck, WriteNack, sender())))
        else
          sender() ! WriteAck
      case None ⇒
    }
    ...
    val newRemovedNodes =
      dataEntries.foldLeft(Set.empty[UniqueAddress]) {
        case (acc, (_, (envelope @ DataEnvelope(data: RemovedNodePruning, _), _))) ⇒
          acc union data.usingNodes.filterNot(n ⇒ n == selfUniqueAddress || knownNodes(n.address))
        case (acc, _) ⇒
          acc
      }
